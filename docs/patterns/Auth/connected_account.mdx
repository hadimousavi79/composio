...
- Labels can be assigned to manage and organize multiple connections for the same user (e.g., when a user connects multiple accounts of the same service), learn more about [using labels](/patterns/Auth/using-labels).
...
- `labels`: labels to be assigned to the connection, which can be used to differentiate between multiple connections for the same entity
...
connection_request = toolset.initiate_connection(
    ...
    entity_id=entity_id, 
    app=App.GMAIL,
    #labels=["primary", "<label-name>"] #labels can be assigned to differentiate between multiple connections for the same entity
)
```
```javascript JavaScript
// Store connection parameters
